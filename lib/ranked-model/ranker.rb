    ...
    private
      ...
      def finder
        @finder ||= begin
          ...
          columns = [_table[instance_class.primary_key], _table[ranker.column]]
          if ranker.scope
            _finder = _finder.send ranker.scope
          end
          case ranker.with_same
            when Symbol
              columns << _table[ranker.with_same]
              _finder = _finder.where _table[ranker.with_same].eq(instance.attributes["#{ranker.with_same}"])
            when Array
              ranker.with_same.each {|c| columns << _table[c] }
              _finder = _finder.where(
                ranker.with_same[1..-1].inject(
                  _table[ranker.with_same.first].eq(instance.attributes["#{ranker.with_same.first}"])
                ) {|scoper, attr|
                  scoper.and(_table[attr].eq(instance.attributes["#{attr}"]))
                }
              )
          end
          if !new_record?
            _finder = _finder.where _table[instance_class.primary_key].not_eq(instance.id)
          end
          _finder.order(_table[ranker.column].asc).select(columns)
        end
      end
